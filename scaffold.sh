...
# 4. Ensure correct imports exist at top of routes.py
if ! grep -q "render_template" "$ROUTES_FILE"; then
  sed -i "1 s/from flask import /from flask import render_template, /" "$ROUTES_FILE"
fi
if ! grep -q "main_bp = Blueprint" "$ROUTES_FILE"; then
  echo "ERROR: main_bp must be defined in routes.py before running this script." >&2
  exit 1
fi
if ! grep -q "from app.models import Task" "$ROUTES_FILE"; then
  sed -i "/^from app import db/a from app.models import Task" "$ROUTES_FILE"
fi

# 5. Append routes *after* main_bp definition
echo "[*] Inserting new routes inside routes.py..."
INSERT_POINT=$(grep -n "main_bp = Blueprint" "$ROUTES_FILE" | cut -d: -f1)
ROUTE_CODE="\n
@main_bp.route('/about')
def about():
...
"
sed -i "$((INSERT_POINT + 1))i$ROUTE_CODE" "$ROUTES_FILE"
